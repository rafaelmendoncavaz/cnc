...
//==================================================================
caminho = get_absolute_file_path("b-q3.sce")
...
x = 5.0 // valor a ser calculado

// Calcular derivadas exatas: https://www.derivative-calculator.net/
// Derivadas exatas no ponto de interesse da função = sqrt(x^3+9)

// f(t)
deff('y = f(x)', 'y = 5 * sin(x) + x^2')
// f'(t)
deff('y = f1(x)', 'y = 5 * cos(x) + 2 * x')
// f''(t)
deff('y = f2(x)', 'y = 2 - 5 * sin(x)')
// f'''(t)
deff('y = f3(x)', 'y = -5 * cos(x)')
// f''''(t)
deff('y = f4(x)', 'y = 5 * sin(x)')

derivada_1a_exata = f1(x) // 11.41831
derivada_2a_exata = f2(x) // 6.794621
derivada_3a_exata = f3(x) // -1.41831
derivada_4a_exata = f4(x) // -4.79462
